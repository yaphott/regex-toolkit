...
class TestStringMethods(unittest.TestCase):
    def test_iter_sort_by_len(self):
        ...
        for try_type, texts_as_try_type in {
            ...
            result = rtk.iter_sort_by_len(texts_as_try_type, reverse=False)
        ...
        for try_type, texts_as_try_type in {
            ...
            result = rtk.iter_sort_by_len(texts_as_try_type, reverse=True)
    ...
    def test_sort_by_len(self):
        ...
        for try_type, texts_as_try_type in {
            ...
            # Not reversed (shortest to longest)
            result = rtk.sort_by_len(texts_as_try_type, reverse=False)
        ...
        for try_type, texts_as_try_type in {
            ...
            # Not reversed (longest to shortest)
            result = rtk.sort_by_len(texts_as_try_type, reverse=True)
    ...
    def test_string_as_exp_safe_chars(self):
        text = "".join(rtk._safe_chars)
        actual_exp = rtk.string_as_exp(text)
        expected_exp = "".join(rtk._safe_chars)
        self.assertEqual(actual_exp, expected_exp)

    def test_string_as_exp2_escapable_chars(self):
        text = "".join(rtk._escapable_chars)
        actual_exp = rtk.string_as_exp2(text)
        expected_exp = "\\" + "\\".join(rtk._escapable_chars)
        self.assertEqual(actual_exp, expected_exp)
    ...
    def test_iter_char_range(self):
        result = rtk.iter_char_range("a", "z")
    ...
    def test_mask_span(self):
        ...
        for try_type, indexes_as_try_type in {
            ...
            actual_text = rtk.mask_span(text, indexes_as_try_type, "not ")
        ...
        for try_type, indexes_as_try_type in {
            ...
            actual_text = rtk.mask_span(text, indexes_as_try_type, "isn't")
    ...
    def test_char_as_exp(self):
        for char, expected_exp in (
            ...
            actual_exp = rtk.char_as_exp(char)
            self.assertEqual(actual_exp, expected_exp)
    ...
    def test_char_as_exp2(self):
        for char, expected_exp in (
            ...
            actual_exp = rtk.char_as_exp2(char)
            self.assertEqual(actual_exp, expected_exp)
    ...
    def test_strings_as_exp(self):
        ...
        for i in range(4):
            for char_tuple in product(i * ["a", "b", "0", "1"]):
                actual_exp = rtk.strings_as_exp(char_tuple)
        ...
        for text in {r"\w", r"\W", r"\d", r"\D", r"\s", r"\S", r"\1"}:
            actual_exp = rtk.strings_as_exp([text])
            expected_exp = f"\\{text}"
            self.assertEqual(actual_exp, expected_exp)
        ...
        for texts, expected_exp in [
            ...
            actual_exp = rtk.strings_as_exp(texts)
            self.assertEqual(actual_exp, expected_exp)
    ...
    def test_strings_as_exp2(self):
        ...
        for i in range(4):
            for char_tuple in product(i * ["a", "b", "0", "1"]):
                actual_exp = rtk.strings_as_exp2(char_tuple)
                expected_exp = "|".join(char_tuple)
                self.assertEqual(actual_exp, expected_exp)
        ...
        # E.g. Should match '\\' + 'd', not r'\d'
        for text in {r"\w", r"\W", r"\d", r"\D", r"\s", r"\S", r"\1"}:
            actual_exp = rtk.strings_as_exp2([text])
            expected_exp = f"\\{text}"
            self.assertEqual(actual_exp, expected_exp)
        ...
        for texts, expected_exp in [
            ...
            actual_exp = rtk.strings_as_exp2(texts)
            self.assertEqual(actual_exp, expected_exp)
